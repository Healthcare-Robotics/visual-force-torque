...
def graph_mag_scatterplot(folder):
    ...
    # pred = pred[gt_initial_mag > 0.5, :]
    # gt = gt[gt_initial_mag > 0.5, :]
    ...
    h = plt.hist2d(gt_mags, pred_mags, bins=(nbins, nbins), cmap=my_cmap, range=[[xmin, xmax], [ymin, ymax]], norm=mcolors.LogNorm(), cmin=5)
    # h = plt.hist2d(gt_mags, thetas, bins=(nbins, nbins), cmap=my_cmap, range=[[0, 10], [0, 180]], cmin=10)
    plt.plot([xmin, xmax], [ymin, ymax], ls="--", c='r', lw=2)


    cbar = plt.colorbar(h[3], ax=plt.gca())
    plt.xlabel('Ground Truth Force Magnitude (N)')
    plt.ylabel('Predicted Force Magnitude (N)')
    # plt.title('Predicted vs Ground Truth Force Magnitude')
    plt.annotate("$r^2 =$ {:.3f}".format(r2_score(gt_mags, pred_mags)), textcoords='axes fraction', xytext=(0.15, 0.8), xy=(0.15,0.8), fontsize=7, color='white')

    
    plt.savefig('./paper/figures/scatterplots/overall_mag_2d_hist_' + args.config + '.png', dpi=500)
    plt.show()
    plt.clf()
    plt.cla()
